...
## Session: 2026-09-10/11 — production-drift discovery, F35 fixed, F46 pipeline restricted to a curated vocabulary and run against real production data, F47 started

### The recent-100 batch's review artifact was broken, and that led to a much bigger discovery
User reported "no tags show up" in the review artifact — a JS bug (`.length` called on `null` for the "no fit" rows, which had `new_tags: null`, throwing before any row rendered). Fixed, but along the way the user asked to skip the artifact-review step and just apply the recent-100 batch directly. Applying it (with a line-ending bug of my own along the way — Python's default text-mode read/write silently converts CRLF↔LF, making every line of a CRLF file show as changed; fixed by reading/writing with `newline=''`) surfaced that 3 files used YAML block-list tag syntax (`tags:\n- a\n- b`) that a naive regex replace didn't fully consume, leaving duplicated tag lines — fixed and verified all 100 files re-parse as valid YAML with exactly the intended tags.

Then, mid-session, the user asked to run the batch **against production**, not local content — and investigation confirmed why that distinction actually matters: `/data/content` (the fly.io volume, the live site's real source of truth) is seeded from git **once**, at first deploy, and nothing ever syncs local/GitHub content back into it afterward except `pages/` (rsync'd on every boot). `content/blog/` sync is one-way, volume→GitHub, via the scheduled `sync_to_github` job. This meant **every prior tag-cleanup batch (1-6, all done against local `content/blog/`) never reached the live site** — confirmed by running the pipeline against production and seeing it start completely fresh from the oldest posts, as if none of that work had happened. Worse: production's live content had itself drifted from local/git in ways beyond tags (one post, `2026-09-08-how-i-code-today.md`, had a full real article in production vs. a one-line stub in the git repo) — meaning the recent-100 batch's own tag decisions had been made against stale content for at least 4 files. Per the user's direction, the recent-100 batch was fully discarded rather than salvaged.

### F35 (stale GIT_TOKEN) diagnosed for real and fixed
While setting up to run batches against production via `fly ssh console`, the sync job failed with `git push` returning "Invalid username or token" — not F35's originally-hypothesized cause (a stale remote URL baked in before a token rotation, fixable by restart alone). Explicitly re-set the remote URL from the *current* env var and it still failed, proving the token itself was invalid on GitHub's side. User generated a new token and ran `fly secrets set GIT_TOKEN=...` themselves; the resulting automatic restart also happened to refresh the container's git remote. `sync_to_github()` now verified working end-to-end (production → GitHub) across every batch run this session. F35's actual code fix (reconstructing the remote URL from the env var at sync time, so a future rotation doesn't need a restart) was discussed but not implemented — still open.

### F46 pipeline: three real design corrections, then run for real
The user rewrote `02-doc/tag-hints.md` from scratch into a rules-first format (Curated Tags / Proposed Tags / GOAL-MANDATORY-JUDGEMENT rules), reversing TF46.1's earlier "tags are free-form" decision — the ending state is now that every *new* tag comes from a single curated list; a non-curated tag recurring 3+ times across a batch queues into "Proposed Tags" for approval instead of being applied (`split_new_tags`/`build_proposal`/`record_recurring_candidates`, `MIN_RECURRENCE_FOR_PROPOSED_TAG`). Existing tags are still never removed regardless of curated-list membership — that rule stayed absolute.

A second, related gap: `BLOG_TAGS` (`utils.py`, F42/F45's post-editor-picklist/MarsEdit-category list) was a *second*, separately-maintained curated list that had drifted from tag-hints.md's new one (missing general categories like `technology`/`personal`). User: "there should be one list." Consolidated — `load_curated_tags()`/`parse_tag_hints_section()` moved to `utils.py` as the one shared parser of `02-doc/tag-hints.md`; `BLOG_TAGS` deleted entirely; `server.py`/`blogger_api.py` call the shared loader directly.

A third correction, found empirically: batch 9's candidate selection overlapped batch 8's by exactly 340 files — batch 8's *entire* no-fit count — because an empty `tags: []` still counted as "needs cleanup" forever, so every already-reviewed thin post kept cycling back into every future batch. Fixed with a `tag_review: no_fit` frontmatter marker (set when a post's final tags end up empty, cleared if real tags are applied later; numeric junk still always needs cleanup regardless) rather than a separate progress file, keeping TF46.0's original "no saved progress state" design intact. Backfilled retroactively for batches 7+8 using ground truth (current tags on disk, not the old proposals JSON — which had partly been wiped from the box's ephemeral storage by the GIT_TOKEN-rotation restart) — dropped the "needs cleanup" count from 2,385 to 2,045 immediately, confirming the fix.

Also caught and corrected a factual error in this session's own reporting: batch 7 was first summarized as "399/500 real tags, 101 no fit," which was actually the raw-decision count *before* curated-vocabulary filtering — the real applied result (checked via ground truth while backfilling the no-fit marker) was 266/500 for batch 7 alone, 426/766 for the batches 7+8 union once accounting for batch 8 re-deciding 234 of batch 7's no-fit posts.

**Batches run against production** (each: select via `fly ssh console`, decisions made by 5 parallel subagents per 500-post batch reading real title/excerpt against the curated list + Proposed Tags queue, `build_proposals.py`/`apply_tag_batch.py` on the box against `/data/content/blog`, `sync_to_github()`, pulled down locally to keep git in sync):

| Batch | Era | Real tags | No fit | New Proposed Tags this batch |
|---|---|---|---|---|
| 7 | 2004-2006 | 266/500 | 234 | blogbridge, blogging, etech, folksonomy, gmail, java, microsoft, orkut, podcasting, red-sox, security, spam, tivo, web2.0 |
| 8 | 2004-2007 | 160/500 | 340 | agile, geek-dinner, gnomedex, howard-stern, opml, social-networking |
| 9 | 2007-2009 | 353/500 | 147 | (none new) |
| 10 | 2009-2013 | 371/500 | 129 | games, theatre |
| 11 | 2013-2021 | 315/500 | 185 | css, flask, git, kubernetes, raspberry-pi, startup, woodworking |
| 12 | 2021-2023 | 392/500 | 108 | excel, graphql, portfolio, postgres, sqlite |

~545 posts remained needing cleanup after batch 11 (2013-2023 era covered by batch 12); the corpus's most recent posts (2023-2026) are still ahead in the self-tracking selection order. `tag-hints.md` also got real usage counts annotated onto every Curated/Proposed entry (scanned all 2,829 posts' actual frontmatter) at the user's request, mid-pipeline — confirmed every Proposed Tag currently shows 0 real uses (never applied, as designed) and surfaced several curated tags with zero uses so far (`chat-gpt`, `claude-code`, `ipad`, `ios`, `linux`, `unix`, `numpy`, `pandas`, `rust`, `llm`, `datarss`).

### F47 — Console theme visual refresh, started
User handed off an external design spec (monospace UI, hairline rules, one amber accent, dark/light toggle) for the public-facing site. Investigated which templates are actually live (`generator.py`/`server.py`) versus dead (`admin_new.html` — never rendered by any route; `overview.html` — rendered but orphaned, not in nav, its would-be homepage variant `generate_overview_as_home` never called) before scoping. Feature/task files written (`03-features/notdone/F47-*.md`, `04-tasks/notdone/TF47-*.md`, 13 tasks) and stopped for approval per the process gate, as required — scope is `base.html` + the 9 live public templates; `admin.html`/`admin_login.html`/`post_editor.html`/`stats_page.html` (standalone, Bootstrap, no design reference given) explicitly excluded. User asked for the work to happen on a dedicated branch rather than this repo's usual direct-to-`main` workflow, given the size of the change — added as an explicit first/last task (create branch / merge back only after visual QA passes). No code written yet.
