...
class TestScorePost:
    def test_recent_post_excluded(self):
        ...
        assert (
            score_post("AI and robots", "Some content about ai", recent, today) is None
        )

    def test_old_post_included(self):
        today = date(2024, 6, 1)
        assert (
            score_post("Just a post", "Some content here", date(2022, 1, 1), today)
            is not None
        )
    ...
    def test_longer_post_scores_higher(self):
        today = date(2024, 6, 1)
        old = date(2022, 1, 1)
        assert score_post("Title", "Short.", old, today) < score_post(
            "Title", "Long. " * 100, old, today
        )
...
class TestGetProposedPosts:
    ...
    def test_excludes_recent_posts(self, tmp_path):
        ...
        self._write_post(
            blog_dir, "2020-01-01-old.md", "Old post", "2020-01-01", "content"
        )
    ...
    def test_result_has_expected_fields(self, tmp_path):
        ...
        self._write_post(
            blog_dir, "2020-01-01-test.md", "Test Post", "2020-01-01", content
        )
    ...
    def test_excludes_draft_posts(self, tmp_path):
        ...
        self._write_post(
            blog_dir, "2020-01-01-real.md", "Real", "2020-01-01", "content"
        )
        filenames = [p.filename for p in get_proposed_posts(blog_dir, 10)]
...
class TestGetProposedDrops:
    ...
    def test_filters_by_age(self, tmp_path):
        ...
        self._write_drop(
            drops_dir, "new-drop.md", date.today().isoformat(), "https://new.com"
        )
        counter = self._make_counter(
            {
    ...
    def test_result_includes_required_fields(self, tmp_path):
        ...
        drops_dir.mkdir()
        self._write_drop(
            drops_dir, "drop.md", "2020-01-01", "https://x.com", note="my note"
        )
