...
class TestPagesLiveServer:
    ...
    def test_pages_url_contains_menu_and_header(
        self, base_url, check_server_accessible
    ):
    ...
    def test_pages_listing_has_content(self, base_url, check_server_accessible):
        ...
        for card in page_cards[:3]:  # Check first 3 cards
            ...
            if title_element:
                ...
                assert len(title_text) > 0, "Page card should have non-empty title"
            ...
            try:
                ...
                if page_response.status_code == 200:
                    ...
                    title = (
                        ...
                        or page_soup.find("h1")
                        or page_soup.find("h2")
                    )
                    ...
                    assert content_length > 100, (
                        f"Page {link} should have substantial content"
                    )
    ...
    def test_pages_have_markdown_rendering(self, base_url, check_server_accessible):
        ...
        if len(markdown_elements) > 0:
            ...
            assert len(content) > 200, (
                "Should have substantial content even without markdown elements"
            )
            ...
            if css_href:
                ...
                try:
                    ...
                    assert css_response.status_code == 200, (
                        f"CSS file {css_url} should be accessible"
                    )
                    assert "text/css" in css_response.headers.get("content-type", ""), (
                        f"CSS file {css_url} should have correct content-type"
                    )
                except requests.exceptions.RequestException:
                    # CSS might be inline or from CDN, don't fail if some aren't accessible
                    pass

    def test_pages_admin_controls_hidden_by_default(
        self, base_url, check_server_accessible
    ):
    ...
    def test_pages_response_time(self, base_url, check_server_accessible):
        ...
        assert response_time < 5.0, (
            f"Page should load within 5 seconds, took {response_time:.2f}s"
        )
    ...
    def test_pages_mobile_responsive(self, base_url, check_server_accessible):
        ...
        response = requests.get(
            f"{base_url}/pages/", headers=mobile_headers, timeout=10
        )
        soup = BeautifulSoup(response.text, "html.parser")
        ...
        assert soup.find("link", {"rel": "stylesheet"}) is not None, (
            "Should link an external stylesheet"
        )
    ...
    def test_raindrops_url_accessible(self, base_url, check_server_accessible):
        ...
        if response.status_code == 404:
            ...
            if response_no_slash.status_code == 200:
                ...
                print(
                    "⚠️  /raindrops/ returns 404 but /raindrops works - URL routing issue (fixed in local code)"
                )
            else:
                pytest.fail(
                    f"Raindrops URL not accessible: /raindrops/ returns {response.status_code}, /raindrops returns {response_no_slash.status_code}"
                )
        ...
        if not heading_found and title_element:
            ...
            if "link" in title_text:
                heading_found = True
                print(
                    f"✓ Page identified as link blog via title: {title_element.get_text()}"
                )

        if not heading_found:
            print(f"⚠️  Page headings found: {actual_headings}")
            print(
                f"⚠️  Page title: {title_element.get_text() if title_element else 'None'}"
            )
            print(f"⚠️  Page content preview: {soup.get_text()[:200]}...")

        assert heading_found, (
            f"Page should have a heading or title indicating raindrops/link content. Found headings: {actual_headings}"
        )
        ...
        raindrop_indicators = [
            ...
            soup.find_all(
                "div", class_=lambda x: x and "raindrop" in x.lower()
            ),  # Raindrop-specific classes
        ...
        assert total_indicators > 0, (
            "Page should contain structural elements suggesting raindrop content"
        )

        # Check that the page has substantial content
        content_length = len(soup.get_text().strip())
        assert content_length > 200, (
            f"Raindrops page should have substantial content, got {content_length} characters"
        )
