...
class TestScheduler:
    ...
    def temp_dirs(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            yield {"data_dir": data_dir, "app_dir": app_dir, "content_dir": content_dir}
    ...
    def test_get_git_remote_success(self, scheduler):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="origin\tgit@github.com:user/repo.git (fetch)\n"
            )
    ...
    def test_check_git_credentials_with_token(self, scheduler):
        ...
        with (
            patch.dict(os.environ, {"GIT_TOKEN": "fake_token"}),
    ...
    def test_check_git_credentials_with_ssh_key(self, scheduler):
        ...
        with (
            patch.dict(os.environ, {"SSH_PRIVATE_KEY": "fake_key"}),
            patch("subprocess.run") as mock_run,
        ):
            mock_run.return_value = Mock(
                returncode=0, stdout="origin\tgit@github.com:user/repo.git (fetch)\n"
    ...
    def test_has_git_changes_with_changes(self, scheduler):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(
                returncode=0, stdout="content/file1.txt\ncontent/file2.txt\n"
            )
    ...
    async def test_sync_stages_images_alongside_content(self, scheduler):
        ...
        with (
            ...
            assert "static/images/uploads/" in staged_paths, (
                "static/images/uploads/ must be staged"
            )
    ...
    async def test_sync_raindrops_no_new_files(self, scheduler):
        ...
        with (
            patch.dict(os.environ, {"RAINDROP_TOKEN": "fake_token"}),
    ...
    def test_sync_wrapper_git_success(self, scheduler):
        ...
        with (
            patch.object(
                scheduler, "_run_async_in_thread", return_value=True
            ) as mock_run,
            patch.object(scheduler, "_handle_sync_result") as mock_handle,
            patch.object(scheduler, "sync_to_github"),
        ):
    ...
    def test_sync_wrapper_raindrop_success(self, scheduler):
        ...
        with (
            ...
            patch.object(scheduler, "_handle_sync_result") as mock_handle,
            patch.object(scheduler, "sync_raindrops"),
        ):
    ...
    def test_sync_wrapper_startup_cancellation(self, scheduler):
        ...
        with (
            patch.object(scheduler, "_run_async_in_thread", return_value=True),
    ...
    def test_handle_sync_result_success_scheduled(self, scheduler):
        ...
        with patch("salasblog2.scheduler.logger") as mock_logger:
            ...
            mock_logger.info.assert_called_once_with(
                "Scheduled Git sync completed successfully"
            )
    ...
    def test_cleanup_startup_job(self, scheduler):
        ...
        with (
            ...
            mock_logger.info.assert_called_once_with(
                "Startup Git sync job cancelled (one-time only)"
            )
    ...
    def test_start_scheduler_success(self, scheduler):
        ...
        with (
            ...
            scheduler.start_scheduler(
                git_interval_hours=1.0, raindrop_interval_hours=0.5
            )
