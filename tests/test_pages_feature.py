...
class TestPagesFeature:
    ...
    def setup_test_environment(self):
        ...
        self.real_templates_dir = Path.cwd() / "templates"
    ...
    def copy_real_templates(self):
        ...
        if not self.real_templates_dir.exists():
            raise FileNotFoundError(
                f"Real templates directory not found: {self.real_templates_dir}"
            )
...
{page_data["content"]}"""
    ...
    def generate_test_site(self):
        ...
        generator.jinja_env = Environment(
            loader=FileSystemLoader(generator.templates_dir)
        )
        generator.jinja_env.filters["strftime"] = generator.format_date
        generator.jinja_env.filters["dd_mm_yyyy"] = (
            lambda date_str: generator.format_date(date_str, "%d-%m-%Y")
        )
    ...
    def test_each_item_has_title_from_frontmatter(self):
        ...
        # Check that page links exist (use actual page urls from the loaded data)
        for page in pages:
    ...
    def test_admin_not_logged_in_no_edit_buttons(self):
        ...
        assert "d-none" in content  # But should be hidden by default
        assert "btn btn-primary btn-sm" in content  # Edit button
        assert "btn btn-danger btn-sm" in content  # Delete button
    ...
    def test_admin_logged_in_shows_edit_delete_buttons(self):
        ...
        assert (
            "checkAdminStatus" in content
        )  # JavaScript function to check admin status
        assert "btn btn-primary btn-sm" in content  # Edit button
        assert "btn btn-danger btn-sm" in content  # Delete button
    ...
    def test_individual_pages_have_admin_edit_delete_buttons(self):
        ...
        for page in pages:
            ...
            assert "admin-controls" in content, (
                f"Page {page_url} should have admin controls"
            )
            assert "btn btn-primary btn-sm" in content, (
                f"Page {page_url} should have edit button"
            )
            assert "btn btn-danger btn-sm" in content, (
                f"Page {page_url} should have delete button"
            )

            # Check that the edit button links to the correct edit page
            assert f"/admin/edit-page/{page['filename']}" in content, (
                f"Page {page_url} should have correct edit link"
            )

            # Check that controls are hidden by default
            assert "d-none" in content, (
                f"Page {page_url} admin controls should be hidden by default"
            )

            # Check that JavaScript will show controls when authenticated
            assert "checkAdminStatus" in content, (
                f"Page {page_url} should have admin status check"
            )
    ...
    def test_admin_logged_in_shows_new_page_button(self):
        ...
        # Check that the new post button structure exists (shared nav for all content types)
        assert "admin-new-post" in content  # CSS class for the new post button in nav
        assert "d-none" in content  # Should be hidden by default, shown by JS
    ...
    def test_pages_with_no_admin_password(self):
        ...
        assert (
            admin_data["authenticated"] is True
        )  # Should be true when no password is set
...
This is a test page with **markdown** formatting."""
    ...
    def test_individual_page_delete_button_uses_correct_function(self):
        ...
        assert "btn btn-danger btn-sm" in content
        assert f"deletePage('{pages[0]['filename']}', this)" in content
    ...
    def test_pages_listing_has_admin_controls(self):
        ...
        edit_button_count = content.count("btn btn-primary btn-sm")
        delete_button_count = content.count("btn btn-danger btn-sm")
        ...
        for page in pages:
            assert f"deletePage('{page['filename']}', this)" in content
    ...
    def test_new_page_button_in_navigation(self):
        ...
        # Check that the new post nav button exists (shared across content types)
    ...
    def test_delete_page_javascript_function_exists(self):
        ...
        # Check that deletePage is called in the template
        assert "deletePage(" in content
    ...
    def test_admin_controls_use_css_classes_not_inline_styles(self):
        ...
        # Check that the admin buttons use Bootstrap classes (no inline styles)
        assert "btn btn-primary btn-sm" in content
        assert "btn btn-danger btn-sm" in content
    ...
    def test_admin_controls_consistent_across_pages_and_individual_pages(self):
        ...
        assert "btn btn-primary btn-sm" in listing_content
        assert "btn btn-primary btn-sm" in individual_content
        assert "btn btn-danger btn-sm" in listing_content
        assert "btn btn-danger btn-sm" in individual_content
        ...
        assert "d-none" in listing_content
        assert "d-none" in individual_content
...
class TestPagesAdminFunctionality:
    ...
    def setup_admin_functionality_test(self):
        ...
        if server_templates_dir.exists():
            config["jinja_env"] = Environment(
                loader=FileSystemLoader(server_templates_dir)
            )
...
"""
    ...
    def test_edit_button_on_page_index_works(self):
        ...
        for page in pages:
            ...
            assert edit_link in content, (
                f"Edit link for {page['filename']} should be in pages listing"
            )
            ...
            assert edit_response.status_code in [200, 302], (
                f"Edit page for {page['filename']} should be accessible or redirect"
            )

            # If it's a redirect, it should be to admin login
            if edit_response.status_code == 302:
                assert "/admin" in edit_response.headers.get("location", ""), (
                ...
                if "Admin Login" in edit_content:
                    ...
                    assert 'name="password"' in edit_content, (
                        "Login form should have a password field"
                    )
                else:
                    # This should be the edit form
                    assert 'name="title"' in edit_content, (
                        "Edit form should have a title field"
                    )
                    assert 'name="content"' in edit_content, (
                        "Edit form should have a content field"
                    )
    ...
    def test_delete_button_on_page_index_works(self):
        ...
        for page in pages:
            delete_call = f"deletePage('{page['filename']}', this)"
            assert delete_call in content, (
                f"Delete button for {page['filename']} should call deletePage function"
            )

            # Ensure it's not calling deletePost
            wrong_delete_call = f"deletePost('{page['filename']}')"
            assert wrong_delete_call not in content, (
                f"Delete button should not call deletePost for {page['filename']}"
            )

        # Check that deletePage is referenced in the template
        assert "deletePage(" in content, "deletePage should be called in the template"
    ...
    def test_edit_and_delete_buttons_on_individual_pages_work(self):
        ...
        for page in pages:
            ...
            response = self.client.get(page_url)
            assert response.status_code == 200, (
                f"Individual page {page_url} should be accessible"
            )
            ...
            edit_link = f"/admin/edit-page/{page['filename']}"
            assert edit_link in content, (
                f"Edit link should be present on individual page {page_url}"
            )

            # Test that edit endpoint responds (may redirect to login, but should not 404)
            edit_response = self.client.get(edit_link)
            assert edit_response.status_code in [200, 302], (
                f"Edit endpoint should work or redirect for {page['filename']}"
            )

            # Test delete button
            delete_call = f"deletePage('{page['filename']}', this)"
            assert delete_call in content, (
                f"Delete button should call deletePage on individual page {page_url}"
            )

            # Ensure delete button doesn't call wrong function
            wrong_delete_call = f"deletePost('{page['filename']}')"
            assert wrong_delete_call not in content, (
                f"Delete button should not call deletePost on {page_url}"
            )

            # Check that admin controls are properly structured
            assert "admin-controls" in content, (
                f"Admin controls should be present on {page_url}"
            )
            assert "btn btn-primary btn-sm" in content, (
                f"Edit button should be present on {page_url}"
            )
            assert "btn btn-danger btn-sm" in content, (
                f"Delete button should be present on {page_url}"
            )
    ...
    def test_create_new_page_form_styling_and_functionality(self):
        ...
        assert response.status_code in [200, 302], (
            "New page form should be accessible or redirect to login"
        )
        ...
        for indicator in form_styling_indicators:
            assert indicator in content, (
                f"Form should contain {indicator} for proper styling"
            )
        ...
        assert "<header" in content or "<nav" in content, (
            "Page should have navigation header"
        )
    ...
    def test_new_page_form_matches_new_post_styling(self):
        ...
        assert new_page_response.status_code in [200, 302], (
            "New page form should be accessible or redirect"
        )
        assert new_post_response.status_code in [200, 302], (
            "New post form should be accessible or redirect"
        )
        ...
        for element in common_elements:
            assert element.lower() in page_content.lower(), (
                f"New page form should have {element}"
            )
            assert element.lower() in post_content.lower(), (
                f"New post form should have {element}"
            )
        ...
        assert len(common_classes) > 0, (
            "New page and new post forms should share some CSS classes"
        )
        ...
        for element in nav_elements:
            page_has_nav = any(
                f"<{element}" in page_content.lower() for element in nav_elements
            )
            post_has_nav = any(
                f"<{element}" in post_content.lower() for element in nav_elements
            )

            if page_has_nav or post_has_nav:
                assert page_has_nav and post_has_nav, (
                    "Both forms should have consistent navigation structure"
                )
    ...
    def test_admin_controls_visibility_consistency(self):
        ...
        # Should have admin controls and they should be hidden via d-none
        assert admin_controls_count > 0, "Pages listing should have admin controls"
        assert "d-none" in listing_content, "Admin controls should be hidden by default"
        ...
        for page in pages:
            ...
            assert "d-none" in content, (
                f"Admin controls on {page_url} should be hidden by default"
            )
    ...
    def test_admin_navigation_buttons_styling_consistency(self):
        ...
        # Check for new post button in nav (shared across content types)
        assert "admin-new-post" in content, "Should have new post button container"

        # Button should be hidden by default
        assert content.count("admin-new-post") > 0 and "d-none" in content, (
            "New post button should be hidden by default"
        )
        ...
        for url in production_urls:
            try:
                ...
                assert response.status_code in [200, 301, 302, 401, 403, 404], (
                    f"Expected valid response code from {url}, got {response.status_code}"
                )
                ...
                if response.status_code == 200:
                    ...
                    if "Admin Login" in content or "password" in content.lower():
                        ...
                        assert "brandeis" in content.lower(), (
                            "Edit form should contain reference to brandeis page"
                        )
                    ...
                    assert "/admin" in location or "/login" in location, (
                    ...
                    print(
                        f"✓ {url} returns 404 for unauthenticated request (as expected)"
                    )
    ...
    def test_production_edit_page_endpoint_with_admin_login(self):
        ...
        if not admin_password:
            ...
            pytest.skip(
                "ADMIN_PASSWORD environment variable not set - skipping authenticated test"
            )

        # Test both production endpoints
        production_urls = ["https://salas.com", "https://salasblog2.fly.dev"]
        ...
        for base_url in production_urls:
            try:
                ...
                if login_response.status_code == 401:
                    ...
                    pytest.skip(
                        f"Login failed for {base_url} - authentication failed (401). Check ADMIN_PASSWORD environment variable."
                    )

                assert login_response.status_code in [200, 302], (
                    f"Login failed for {base_url}, got {login_response.status_code}"
                )
                ...
                assert edit_response.status_code in [200, 302], (
                    f"Edit page request failed for {edit_url}, got {edit_response.status_code}"
                )
                ...
                if edit_response.status_code == 200:
                    ...
                    assert 'name="title"' in content, (
                        f"Edit form should have title field for {base_url}"
                    )
                    assert 'name="content"' in content, (
                        f"Edit form should have content field for {base_url}"
                    )
                    assert 'name="date"' in content, (
                        f"Edit form should have date field for {base_url}"
                    )

                    # Should contain reference to the brandeis page
                    assert "brandeis" in content.lower(), (
                        f"Edit form should reference brandeis page for {base_url}"
                    )

                    # Should have proper form styling
                    assert "admin-forms.css" in content, (
                        f"Edit form should include admin forms CSS for {base_url}"
                    )

                    # Should have edit-specific elements
                    assert "Edit Page" in content or "edit" in content.lower(), (
                        f"Edit form should indicate it's editing for {base_url}"
                    )

                    # Should have save/update button
                    assert "Save" in content or "Update" in content, (
                        f"Edit form should have save/update button for {base_url}"
                    )
                    ...
                    print(
                        f"✓ {base_url} edit page redirects to {location} (acceptable)"
                    )
                ...
                if status_response.status_code == 200:
                    ...
                    assert status_data.get("authenticated"), (
                        f"Admin status should show authenticated for {base_url}"
                    )
                ...
                assert False, (
                    f"Unexpected error testing authenticated production endpoint {base_url}: {e}"
                )
            ...
            try:
                ...
                for page in pages_to_test:
                    ...
                    if edit_response.status_code in [200, 302]:
                        ...
                        try:
                            error_detail = edit_response.json().get(
                                "detail", "Unknown error"
                            )
                            failed_pages.append(f"{page}: {error_detail}")
                            print(
                                f"✗ {page} edit endpoint failed for {base_url}: {error_detail}"
                            )
                        except Exception:
                            failed_pages.append(f"{page}: Server error (500)")
                            print(
                                f"✗ {page} edit endpoint failed for {base_url}: Server error (500)"
                            )
                    else:
                        failed_pages.append(f"{page}: HTTP {edit_response.status_code}")
                        print(
                            f"✗ {page} edit endpoint failed for {base_url}: HTTP {edit_response.status_code}"
                        )
                ...
                assert edit_response.status_code in [200, 302], (
                    f"Page edit request failed for {base_url}, got {edit_response.status_code}"
                )
                ...
                if edit_response.status_code == 200:
                    ...
                    assert 'name="title"' in content, (
                        f"Page edit form should have title field for {base_url}"
                    )
                    assert 'name="content"' in content, (
                        f"Page edit form should have content field for {base_url}"
                    )
                    assert 'name="date"' in content, (
                        f"Page edit form should have date field for {base_url}"
                    )

                    # Should have proper form styling
                    assert "admin-forms.css" in content, (
                        f"Page edit form should include admin forms CSS for {base_url}"
                    )

                    # Should have edit-specific elements
                    assert "Edit Page" in content or "edit" in content.lower(), (
                        f"Page edit form should indicate it's editing for {base_url}"
                    )

                    # Should have save/update button
                    assert "Save" in content or "Update" in content, (
                        f"Page edit form should have save/update button for {base_url}"
                    )

                    print(
                        f"✓ {base_url} page {working_page} can be edited successfully"
                    )
                    ...
                    location = edit_response.headers.get("location", "")
                    print(
                        f"✓ {base_url} page edit redirects to {location} (acceptable)"
                    )
                ...
                assert page_response.status_code == 200, (
                    f"Page should be accessible at {page_url}"
                )

                # Step 4: Check that the page has edit controls in the HTML
                page_content = page_response.text
                assert "admin-btn-edit" in page_content, (
                    f"Page should have edit button for {base_url}"
                )
                assert f"admin/edit-page/{working_page}" in page_content, (
                    f"Page should have correct edit link for {base_url}"
                )

                print(
                    f"✓ {base_url} page {page_name} is accessible and has edit controls"
                )
                ...
                assert False, (
                    f"Unexpected error testing about page editing for {base_url}: {e}"
                )

        # At least one server should work
        assert successful_tests > 0, (
            "At least one production server should allow page editing"
        )
    ...
    def test_link_blog_functionality(self):
        ...
        assert main_page_content is not None, (
            f"Could not find working main page. Tried: {main_page_urls}"
        )
        ...
        assert (
            "raindrops" in raindrops_content.lower()
            or "link blog" in raindrops_content.lower()
        ...
        if raindrops_dir.exists():
            ...
            if raindrop_files:
                ...
                for raindrop_file in raindrop_files[:3]:  # Check first 3 raindrops
                    ...
                    raindrop_found = (
                        raindrop_name in raindrops_content
                        or f"{raindrop_name}.html" in raindrops_content
                    )
                    ...
                    print("⚠️  No raindrop content found in raindrops page")
                    print(
                        f"   Raindrops page length: {len(raindrops_content)} characters"
                    )
                    print(
                        f"   Available raindrop files: {[f.name for f in raindrop_files]}"
                    )

                assert len(raindrop_files) > 0, (
                    "Should have some raindrop files to test"
                )
                ...
                has_listing_structure = any(
                    indicator in raindrops_content for indicator in listing_indicators
                )
                assert has_listing_structure, (
                    "Raindrops page should have a listing structure"
                )

                print("✓ Raindrops page has proper listing structure")
                print(f"✓ Found {len(raindrop_files)} raindrop files")
            else:
                print("⚠️  No raindrop files found, skipping raindrop content checks")
        else:
            print(
                "⚠️  Raindrops directory doesn't exist, skipping raindrop content checks"
            )
    ...
    def test_link_blog_navigation_from_main_page(self):
            ...
            if matches:
                for match in matches:
                    if "raindrops" in match.lower() or "link blog" in match.lower():
                        found_nav = True
                        print(
                            f"✓ Found link blog/raindrops in navigation: {match[:100]}..."
                        )
                        break
                if found_nav:
                    break
        ...
        if not found_nav:
            ...
            raindrop_links = [
                ...
                if "raindrops" in href.lower()
                or "raindrops" in text.lower()
                or "link" in text.lower()
                or "blog" in text.lower()
            ]
