...
class TestCreateExcerpt:
    ...
    def test_create_excerpt_does_not_split_bold_marker(self):
        ...
        text = (
            "x" * 295
            + "**bold text** more filler well past the smart threshold "
            + "y" * 400
        )
    ...
    def test_create_excerpt_does_not_split_html_tag(self):
        ...
        text = (
            "x" * 295
            + "<strong>bold</strong> more filler well past threshold "
            + "y" * 400
        )
        result = create_excerpt(text, 300, 400)
        assert "<strong" not in result.rstrip(".")

    def test_create_excerpt_strips_markdown_header_marker(self):
        """Regression: a '## Heading' written on its own line in the source
        (confirmed via MarsEdit) only means anything to markdown at the start
        of a line. create_excerpt_with_info collapses newlines into spaces
        before truncating, so the '## ' ended up mid-sentence and printed as
        literal pound-sign characters instead of becoming a heading."""
    ...
    def test_create_excerpt_does_not_strip_bold_marker_mid_line(self):
        """Block-marker stripping only applies at the start of a line — '**bold**'
        appearing mid-sentence must not be mistaken for a list bullet."""
        text = "Some text with **bold** in the middle of a sentence."
        result = create_excerpt(text)
...
""")
        ...
        assert (
            "<h1" in result["html_content"]
            and "This is a test post</h1>" in result["html_content"]
        )
...
class TestGenerateRaindropFilename:
    ...
    def test_generate_raindrop_filename_long_title(self):
        ...
        assert len(result) < len(
            "25-06-28-5-This is a very long title that should be truncated at thirty characters.md"
        )
