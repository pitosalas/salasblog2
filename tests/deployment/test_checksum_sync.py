...
class TestRsyncChecksumBehavior:
    ...
    def test_checksum_ignores_timestamps(self, temp_directories):
        ...
        assert dest_stat_before.st_mtime > source_stat.st_mtime, (
            "Destination should have newer timestamp"
        )

        # Edit source content (but keep older timestamp)
        time.sleep(1)
        create_test_post(
            source_dir, post_file, "EDITED content - should sync despite timestamp!"
        )
        ...
        assert (
            "EDITED content - should sync despite timestamp!" in final_dest_content
        ), "Content should be synced despite newer destination timestamp"
    ...
    def test_identical_content_skipped(self, temp_directories):
        ...
        # Sync again - checksum comparison means content, not mtime, decides
        # whether to transfer data; -a still syncs mtime metadata regardless
        # of whether data was retransferred, so mtime alone can't prove a skip.
        result = run_rsync_checksum(source_dir.parent, dest_dir.parent)
    ...
    def test_content_based_detection(self, temp_directories):
        ...
        # Create initial file
        create_test_post(source_dir, post_file, "Version 1 content")
        run_rsync_checksum(source_dir.parent, dest_dir.parent)
        ...
        assert "Version 2 content" in final_content, (
            "Content should be updated to version 2"
        )
        assert "Version 1 content" not in final_content, (
    ...
    def test_multiple_files_checksum_sync(self, temp_directories):
        ...
        create_test_post(source_dir, "file1.md", "Content A")
        create_test_post(source_dir, "file2.md", "Content B")
        create_test_post(source_dir, "file3.md", "Content C")
    ...
    def test_mars_edit_scenario_simulation(self, temp_directories):
        ...
        source_post = create_test_post(
            source_dir, post_file, "Original blog post content"
        )
        run_rsync_checksum(source_dir.parent, dest_dir.parent)
        ...
        create_test_post(
            source_dir, post_file, "EDITED via MarsEdit - this should sync!"
        )

        # Verify the problematic timestamp situation
        source_stat = source_post.stat()
        dest_stat = dest_post.stat()
        assert source_stat.st_mtime > dest_stat.st_mtime, (
            "This would normally work with --update"
        )
        ...
        assert dest_stat_after.st_mtime > source_stat_after.st_mtime, (
            "Destination is now newer (problematic for --update)"
        )
        ...
        final_content = dest_post.read_text()
        assert "EDITED via MarsEdit - this should sync!" in final_content, (
            "MarsEdit edits should sync despite timestamp conflicts"
        )
        assert "Original blog post content" not in final_content, (
            "Old content should be replaced"
        )
...
class TestRsyncChecksumFeatures:
    ...
    def test_checksum_performance_note(self, temp_directories):
        ...
        assert sync_time < 10, (
            "Checksum sync should complete reasonably quickly for moderate files"
        )
