...
def check_server_running(base_url):
    ...
    try:
        ...
        pytest.skip(
            "Test server not running at localhost:8003. Start with: salasblog2 server --port 8003"
        )
...
class TestServerEndpoints:
    ...
    def test_home_page(self, base_url, check_server_running):
        ...
        assert any(keyword in content for keyword in ["SALAS", "BLOG", "HOME"]), (
            "Home page should contain blog-related content"
        )
    ...
    def test_admin_page(self, base_url, check_server_running):
        ...
        assert "Admin Login" in content or "Admin Panel" in content, (
            "Admin page should show login or panel"
        )
    ...
    def test_static_files(self, base_url, check_server_running):
        ...
        assert found_static, (
            f"At least one static file should be accessible from {static_paths}"
        )
...
class TestServerAPI:
    ...
    def test_scheduler_status(self, base_url, check_server_running):
        ...
        assert "git_configured" in data, (
            "Scheduler status should include git config state"
        )
