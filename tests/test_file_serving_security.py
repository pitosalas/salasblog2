            ...
            if response.status_code == 200:
                ...
                for pattern in sensitive_patterns:
                    assert pattern not in response.text, (
                        f"Symlink exposes system file content: {pattern}"
                    )
        ...
        for path in hidden_files:
            ...
            if response.status_code == 200:
                ...
                assert "SECRET_KEY" not in response.text, (
                    f"Hidden file {path} exposes SECRET_KEY"
                )
                assert "sensitive" not in response.text, (
                    f"Hidden file {path} exposes sensitive content"
                )
...
class TestFileServingEdgeCases:
    ...
    def test_directory_index_handling(self, client, mock_output_dir):
        ...
        # Create index files
        (mock_output_dir / "blog" / "index.html").write_text(
            "<html><body>Blog Index</body></html>"
        )
        (mock_output_dir / "pages" / "index.html").write_text(
            "<html><body>Pages Index</body></html>"
        )
    ...
    def test_catch_all_html_extension_addition(self, client, mock_output_dir):
        ...
        (mock_output_dir / "about.html").write_text(
    ...
    def test_unicode_filenames(self, client, mock_output_dir):
        ...
        try:
            ...
            unicode_files = ["/static/café.css", "/static/测试.js"]
