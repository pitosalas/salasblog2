...
class TestPublishDraft:
    def test_removes_draft_flag(self, client, tmp_path):
        ...
        with (
            patch("salasblog2.server.get_content_directory", return_value=blog_dir),
...
class TestGenerateDraftEndpoint:
    def test_returns_draft_filename(self, client, tmp_path):
        ...
        with (
            patch("salasblog2.server.get_content_directory", side_effect=fake_get_dir),
            patch(
                "salasblog2.server.generate_draft_from_drop", return_value=mock_content
            ),
