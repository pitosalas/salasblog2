...
def create_excerpt_with_info(
    content: str, max_length: int = None, smart_threshold: int = None
) -> tuple[str, bool]:
...
def process_markdown_to_html(content: str) -> str:
    ...
    fixed_content = fixed_content.replace(
        " ](", "]("
    )  # Remove space before closing bracket and opening paren
    fixed_content = fixed_content.replace(
        "]( ", "]("
    ...
    try:
        ...
        if post.content and len(post.content) > 0:
            ...
            if post.content.startswith("<"):
                _logger.warning(
                    f"File {file_path.name} contains HTML, not markdown: {content_start}"
                )
...
def _parse_malformed_frontmatter(
    file_path: Path, original_error: Exception
) -> Dict[str, Any]:
...
def _create_fallback_metadata(file_path: Path, content: str) -> Dict[str, Any]:
    ...
    title = (
        filename_stem.replace("-", " ").replace("_", " ").title()
        if "-" in filename_stem or "_" in filename_stem
        else filename_stem
    )

    return {
        "metadata": {"title": title, "date": "", "type": "blog"},
        "content": content,
        "raw_content": content,
        "html_content": process_markdown_to_html(content),
    }
...
def sort_posts_by_date(
    ...
    return sorted(
        posts,
        key=lambda post: parse_date_for_sorting(post.get("date")),
        reverse=reverse,
    )
...
def format_raindrop_as_markdown(raindrop: Dict[str, Any]) -> str:
    ...
    frontmatter_data = {
        ...
        "collection_id": raindrop.get("collection", {}).get("$id")
        if isinstance(raindrop.get("collection"), dict)
        else None,
        # User info
        "user_id": raindrop.get("user", {}).get("$id")
        if raindrop.get("user")
        else None,
        # Media/covers
