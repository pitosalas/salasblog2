#!/usr/bin/env python3
# scheduler — Background thread scheduling periodic GitHub and Raindrop.io sync
...
class Scheduler:
    ...
    def _run_command_with_retry(
        self, cmd: List[str], retries: int = 2, timeout: int = 60
    ...
    def _commit_and_push(self) -> bool:
        ...
        try:
            ...
            self._run_command_with_retry(
                ["git", "commit", "-m", commit_message], timeout=30
            )
    ...
    async def sync_to_github(self) -> bool:
        ...
        try:
            ...
            try:
                self._run_command_with_retry(
                    ["git", "fetch", "origin", self.config.git_branch], timeout=30
                )
                ...
                if merge.returncode != 0:
                    self._log_error(
                        f"Cannot fast-forward before push — diverged history: {merge.stderr.strip()}"
                    )
                    return False
            except Exception as e:
    ...
    def start_scheduler(
        self, git_interval_hours: float = None, raindrop_interval_hours: float = None
    ):
        ...
        scheduler_thread = Thread(
            target=run_scheduler, daemon=True, name="SyncScheduler"
        )
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "last_git_sync": self.last_git_sync.isoformat()
            if self.last_git_sync
            else None,
            "last_raindrop_sync": self.last_raindrop_sync.isoformat()
            if self.last_raindrop_sync
            else None,
    ...
    def get_detailed_status(self) -> dict:
        ...
        return {
            **self.get_status(),
            ...
            "file_counts": {
                "data_content": len(list(self.config.data_dir.glob("**/*")))
                if self.config.data_dir.exists()
                else 0,
                "app_content": len(list(self.config.content_dir.glob("**/*")))
