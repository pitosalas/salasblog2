#!/usr/bin/env python3
# generator — Static site generator: renders markdown+frontmatter content into HTML
...
class SiteGenerator:
    def __init__(self):
        ...
        self.jinja_env.filters["dd_mm_yyyy"] = lambda date_str: format_date(
            date_str, "%d-%m-%Y"
        )
    ...
    def generate_search_index(self, all_posts):
        ...
        for post in all_posts:
            search_item = {
                ...
                if len(post["raw_content"]) > 500
                else post["raw_content"],
            }
    ...
    def copy_static_files(self):
        ...
        if volume_uploads.exists():
            ...
            print("✓ Restored uploaded images from volume")
    ...
    def generate_listing_pages(
        ...
        total_pages = max(
            1, (total_posts + posts_per_page - 1) // posts_per_page
        ...
        if target_filename is not None:
            ...
            try:
                post_index = next(
                    i for i, p in enumerate(posts) if p.get("filename") == bare
                )
        ...
        for page_num in range(1, total_pages + 1):
            ...
            pagination = {
                ...
                "prev_url": self._get_page_url(content_type, page_num - 1)
                if page_num > 1
                else None,
                "next_url": self._get_page_url(content_type, page_num + 1)
                if page_num < total_pages
                else None,
                "page_urls": [
                    self._get_page_url(content_type, p)
        ...
        if target_pages is not None:
            print(
                f"✓ Generated {content_type} listing pages ({pages_written} of {total_pages} pages, {total_posts} posts)"
            )
        else:
            print(
                f"✓ Generated {content_type} listing pages ({total_pages} pages, {total_posts} posts)"
            )
    ...
    def generate_collection_filtered_pages(
        self, raindrops, collections, collection_counts=None
    ):
        ...
        for collection in collections:
            ...
            filtered_raindrops = [
                r for r in raindrops if r.get("collection") == collection
            ]
            ...
            for page_num in range(1, total_pages + 1):
                ...
                pagination = {
                    ...
                    "has_prev": page_num > 1,
                    "has_next": page_num < total_pages,
                    "prev_url": self._get_page_url(
                        "raindrops", page_num - 1, collection_slug
                    )
                    if page_num > 1
                    else None,
                    "next_url": self._get_page_url(
                        "raindrops", page_num + 1, collection_slug
                    )
                    if page_num < total_pages
                    else None,
                    "page_urls": [
                        self._get_page_url("raindrops", p, collection_slug)
                        for p in range(1, total_pages + 1)
                    ],
                }
    ...
    def generate_home_page(self, blog_posts, raindrops):
        ...
        sorted_raindrops = sorted(
            raindrops, key=lambda x: x.get("date", ""), reverse=True
        )
    ...
    def generate_site(self):
        ...
        self.generate_listing_pages(
            raindrops, "raindrops", collections, collection_counts
        )
        self.generate_collection_filtered_pages(
            raindrops, collections, collection_counts
        )
        print()
        ...
        print("🌐 Ready to serve or deploy!")

    def incremental_regenerate_post(
        self, post_filename: str, content_type: str = "blog"
    ):
        ...
        changed_post = next(
            (
                ...
                if p["filename"] == post_filename.replace(".md", "")
            ),
            None,
        )
        ...
        if content_type in ["blog", "raindrops"]:
            self.generate_listing_pages(
                changed_posts, content_type, target_filename=post_filename
            )
            print(f"✓ Regenerated {content_type} listing page")

        # Regenerate home page (blog + raindrops only)
        self.generate_home_page(blog_posts, raindrops)
        print("✓ Regenerated home page")

        # Regenerate search index
        self.generate_search_index(blog_posts + raindrops + pages)
        print("✓ Regenerated search index")

        print("✅ Incremental regeneration complete!")

    def incremental_regenerate_after_deletion(
        self, post_filename: str, content_type: str = "blog"
    ):
        """Incrementally regenerate site after a post deletion."""
        print(
            f"🗑️ Incremental regeneration after {content_type} deletion: {post_filename}"
        )
        ...
        if content_type == "pages":
            post_output_file = (
                self.output_dir / f"{post_filename.replace('.md', '')}.html"
            )
        else:
            post_output_file = (
                self.output_dir
                / content_type
                / f"{post_filename.replace('.md', '')}.html"
            )
        ...
        print("✅ Incremental deletion regeneration complete!")
